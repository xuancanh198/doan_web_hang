...
import { PermisstionListTable } from "@/constants/Interface";
import Link from "next/link";
import Swal from "sweetalert2";
import { deletePermisstion } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
import { useSelector, useDispatch } from "react-redux";
import { AppDispatch } from "@/lib/redux/store";
import { RootState } from "@/lib/redux/store";
import { addDeleteId, removeDeleteId, setDeleteIds } from "@/lib/redux/Features/Crud";

interface PropPermisstionTable {
  data?: PermisstionListTable | any[];
}

export default function PermisstionTableFrom({ data = [] }: PropPermisstionTable) {
  ...
  const deleteAction = async (id: number) => {
    const result = await Swal.fire({
      title: t("Base.Delete") + " " + t("Page.permisstion"),
      text: t("Base.TitleDeleteModal",{field : t('Page.permisstion')}),
      icon: "warning",
    ...
    if (result.isConfirmed) {
      await dispatch(deletePermisstion(id));
    }
  };
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Permisstion.name")}</TableCell>
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Permisstion.code")}</TableCell>
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Permisstion.status")}</TableCell>
                ...
                {data.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="px-4 py-3 text-start">
                      ...
                      >
                        {item.status === 1
                          ? t("Permisstion.statusValue.active")
                          : t("Permisstion.statusValue.hideTemporarily")}
                      </Badge>
                    </TableCell>
                    <TableCell className="px-4 py-3 flex gap-2 justify-center">
                      <Link
                        href={`/admin/manage-list/edit/${item.id}?query=permisstion`}
                        className="bg-orange-500 hover:bg-orange-800 text-white px-4 py-2 rounded-lg text-sm"
                      >
                        {t("Base.Update")}
                      </Link>
                      <Link
                        href={`/admin/manage-list/view/${item.id}?query=permisstion`}
