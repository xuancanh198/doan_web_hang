...
import { SettingListTable } from "@/constants/Interface";
import Link from "next/link";
import Swal from "sweetalert2";
import { deleteSetting } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
import { useSelector, useDispatch } from "react-redux";
import { AppDispatch } from "@/lib/redux/store";
import { RootState } from "@/lib/redux/store";
import { addDeleteId, removeDeleteId, setDeleteIds } from "@/lib/redux/Features/Crud";

interface PropSettingTable {
  data?: SettingListTable | any[];
}

export default function SettingTableFrom({ data = [] }: PropSettingTable) {
  ...
  const deleteAction = async (id: number) => {
    const result = await Swal.fire({
      title: t("Base.Delete") + " " + t("Page.Setting"),
      text: t("Base.TitleDeleteModal",{field : t('Page.Setting')}),
      icon: "warning",
    ...
    if (result.isConfirmed) {
      await dispatch(deleteSetting(id));
    }
  };
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Setting.key")}</TableCell>
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Setting.value")}</TableCell>
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Setting.group")}</TableCell>
                ...
                {data.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="px-4 py-3 flex gap-2 justify-center">
                      <Link
                        href={`/admin/manage-list/edit/${item.id}?query=setting`}
                        className="bg-orange-500 hover:bg-orange-800 text-white px-4 py-2 rounded-lg text-sm"
                      >
                        {t("Base.Update")}
                      </Link>
                      <Link
                        href={`/admin/manage-list/view/${item.id}?query=setting`}
