...
import { ActiveLogListTable } from "@/constants/Interface";
import Link from "next/link";
import Swal from "sweetalert2";
import { deleteLogActive } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
import { useSelector, useDispatch } from "react-redux";
import { AppDispatch } from "@/lib/redux/store";
import { RootState } from "@/lib/redux/store";
import { addDeleteId, removeDeleteId, setDeleteIds } from "@/lib/redux/Features/Crud";

interface PropLogActiveTable {
  data?: ActiveLogListTable | any[];
}

export default function LogActiveTableFrom({ data = [] }: PropLogActiveTable) {
  const t = useTranslations("ManageInAdmin");
  const dispatch = useDispatch<AppDispatch>();
  const deleteIds = useSelector((state: RootState) => state.crud.deleteIds);
  const [checkAll, setCheckAll] = useState(false);

  const deleteLogActiveFun = async (id: number) => {
    const result = await Swal.fire({
      title: t("Base.Delete") + " " + t("Page.logActive"),
      text: t("Base.TitleDeleteModal",{field : t('Page.logActive')}),
      icon: "warning",
    ...
    if (result.isConfirmed) {
      await dispatch(deleteLogActive(id));
    }
  };
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 text-start">{t("LogActive.log_name")}</TableCell>
                  <TableCell isHeader className="px-5 py-3 text-start">{t("LogActive.description")}</TableCell>
                  <TableCell isHeader className="px-5 py-3 text-start">{t("LogActive.subject_type")}</TableCell>
                ...
                {data.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="px-4 py-3 flex gap-2 justify-center">
                      <Link
                        href={`/admin/manage-list/view/${item.id}?query=log-active`}
                        className="bg-green-500 hover:bg-green-800 text-white px-4 py-2 rounded-lg text-sm"
                      >
                        {t("Base.View")}
                      </Link>
                      <div
                        onClick={() => deleteLogActiveFun(item.id)}
