...
import { PublisherListTable } from "@/constants/Interface";
import Link from "next/link";
import Swal from "sweetalert2";
import { deletePublisher } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
...
interface PropPublisherTable {
  data?: PublisherListTable | any[];
}

export default function PublisherTableFrom({ data = [] }: PropPublisherTable) {
  ...
  const deleteAction = async (id: number) => {
    const result = await Swal.fire({
      title: t("Base.Delete") + " " + t("Page.publisher"),
      text: t("Base.TitleDeleteModal",{field : t('Page.publisher')}),
      icon: "warning",
              ...
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <div className="flex gap-1 items-center">
                     ...
                     <p> {t("Publisher.code")}</p>
                    </div>
                    </TableCell>
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Publisher.status")}</TableCell>
                ...
                {data.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="px-4 py-3 text-start">
                      <Badge
                        size="sm"
                        color={Number(item.status) === 1 ? "success" : "error"}
                      >
                        {Number(item.status) === 1
                          ? t("Publisher.statusValue.active")
                          : t("Publisher.statusValue.hideTemporarily")}
                      </Badge>
                    </TableCell>
                    <TableCell className="px-4 py-3 flex gap-2 justify-center">
                      <Link
                        href={`/admin/manage-list/edit/${item.id}?query=publisher`}
                        className="bg-orange-500 hover:bg-orange-800 text-white px-4 py-2 rounded-lg text-sm"
                      >
                        {t("Base.Update")}
                      </Link>
                      <Link
                        href={`/admin/manage-list/view/${item.id}?query=publisher`}
