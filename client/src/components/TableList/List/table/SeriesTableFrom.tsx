...
interface PropSeriesTable {
  data?: SeriesListTable | any[];
}

export default function SeriesTableFrom({ data = [] }: PropSeriesTable) {
  ...
  const deleteAction = async (id: number) => {
    const result = await Swal.fire({
      title: t("Base.Delete") + " " + t("Page.series"),
      text: t("Base.TitleDeleteModal",{field : t('Page.series')}),
      icon: "warning",
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Series.status")}</TableCell>
                ...
                {data.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="px-4 py-3 text-start">
                      ...
                      >
                        {item.status === 1
                          ? t("Series.statusValue.active")
                          : t("Series.statusValue.hideTemporarily")}
                      </Badge>
                    </TableCell>
                    <TableCell className="px-4 py-3 flex gap-2 justify-center">
                      <Link
                        href={`/admin/manage-list/edit/${item.id}?query=series`}
                        className="bg-orange-500 hover:bg-orange-800 text-white px-4 py-2 rounded-lg text-sm"
                      >
                        {t("Base.Update")}
                      </Link>
                      <Link
                        href={`/admin/manage-list/view/${item.id}?query=series`}
