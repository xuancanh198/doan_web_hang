...
import { deleteProduct } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
...
interface PropProductTable {
  data?: ProductListTable | any[];
}

export default function ProductTableFrom({ data = [] }: PropProductTable) {
  ...
  const deleteAction = async (id: number) => {
    const result = await Swal.fire({
      title: t("Base.Delete") + " " + t("Page.product"),
      text: t("Base.TitleDeleteModal",{field : t('Page.product')}),
      icon: "warning",
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 text-start">{t("productImportExport.action")}</TableCell>
