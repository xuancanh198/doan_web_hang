...
import { ActionListTable } from "@/constants/Interface";
import Link from "next/link";
import Swal from "sweetalert2";
import { deleteAction } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
import { useSelector, useDispatch } from "react-redux";
import { AppDispatch } from "@/lib/redux/store";
import { RootState } from "@/lib/redux/store";
import { addDeleteId, removeDeleteId, setDeleteIds } from "@/lib/redux/Features/Crud";

interface PropActionTable {
  data?: ActionListTable | any[];
}

export default function ActionTableFrom({ data = [] }: PropActionTable) {
  const t = useTranslations("ManageInAdmin");
  const dispatch = useDispatch<AppDispatch>();
  const deleteIds = useSelector((state: RootState) => state.crud.deleteIds);
  const [checkAll, setCheckAll] = useState(false);

  const deleteActionFun = async (id: number) => {
    const result = await Swal.fire({
      title: t("Base.Delete") + " " + t("Page.action"),
      text: t("Base.TitleDeleteModal",{field : t('Page.action')}),
      icon: "warning",
            ...
            <Table>
              <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                <TableRow>
                  ...
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Action.name")}</TableCell>
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Action.code")}</TableCell>
                  <TableCell isHeader className="px-5 py-3 text-start">{t("Action.status")}</TableCell>
