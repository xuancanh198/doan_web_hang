...
export default function DefaultInputs({ dataDetail }: { dataDetail: any }) {
  ...
  const PermisstionSchema = Yup.object().shape({
    ...
    status: Yup.string()
      .required(t('Validation.RequiredField', { field: t('Permisstion.status') })),
  });
  ...
  const createData = async (values: any, resetForm: () => void) => {
    const data = await dispatch(updatePermisstionDetail(dataDetail?.id, values, true));
  ...
  return (
    <ComponentCard title={t('Base.EditTitle', { field: t('Page.permisstionDetail') })}>
      <Formik
        enableReinitialize
        initialValues={{
          ...
          url: dataDetail?.url || "",
          action_id: dataDetail?.action_id || 1,
          permisstion_id: dataDetail?.permisstion_id || 1,
