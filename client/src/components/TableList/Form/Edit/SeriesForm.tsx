...
import { updateSeries } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
...
export default function DefaultInputs({ dataDetail }: { dataDetail: any }) {
  ...
  const createData = async (values: any, resetForm: () => void) => {
    ...
    const data = await dispatch(updateSeries(dataDetail?.id, formData, true));
  ...
  return (
    <ComponentCard title={t('Base.EditTitle', { field: t('Page.series') })}>
      <Formik
       enableReinitialize
      initialValues={{
