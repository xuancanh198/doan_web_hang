...
export default function DefaultInputs({ dataDetail }: { dataDetail: any }) {
  ...
  const updateData = async (values: any, resetForm: () => void) => {
    ...
    const data = await dispatch(updateBanner(dataDetail?.id ,formData, true));
  ...
  return (
    <ComponentCard title={t('Base.EditTitle', { field: t('Page.banner') })}>
      <Formik
        initialValues={{
          title: dataDetail?.title ?? "",
          status: dataDetail?.status?.toString() ?? STATUS_ACTIVE.toString(),
          link: dataDetail?.link ?? "",
          position: dataDetail?.position ?? 1,
