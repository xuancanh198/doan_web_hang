...
import { updateSetting } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
...
export default function DefaultInputs({ dataDetail }: { dataDetail: any }) {
  ...
  const [optionsSetting, setOptionSetting] = useState<any[]>([]);
 const SettingSchema = Yup.object().shape({
  ...
  group: Yup.string()
    .nullable()
    .max(50, t('Validation.MaxLength', { field: t('Setting.group'), max: 50 })),
  description: Yup.string()
    .nullable()
    .max(500, t('Validation.MaxLength', { field: t('Series.description'), max: 5000 })),
});

  const setFieldValueRef = useRef<any>(null);
  useEffect(() => {
    const callAPiDataAll = async () => {
      const res = await getDataSettingServer(
        false,
        PAGE_DEFAULT,
        LIMIT_DEFAULT,
        NULL_VALUE_DEFAULT,
      ...
      const dataSettingAll = res?.result?.map((item: any, index: number) => {
        return {
          value: item?.id,
          label: item?.name,
        };
      ...
      setOptionSetting(dataSettingAll);
    };
    callAPiDataAll();
  }, [])

  // const options = [
  //   { value: STATUS_ACTIVE, label: t('Setting.statusValue.active') },
  //   { value: STATUS_NOT_ACTIVE, label: t('Setting.statusValue.hideTemporarily') },
  // ];
  ...
  const createData = async (values: any, resetForm: () => void) => {
    ...
    const data = await dispatch(updateSetting(dataDetail?.id ,dataJson, true));
  ...
  return (
    <ComponentCard title={t('Base.EditTitle', { field: t('Page.setting') })}>
      <Formik
        initialValues={{
          ...
          type: dataDetail?.type ??SETTING_TYPE_INPUT_TEXT,
          group: dataDetail?.group ?? null,
