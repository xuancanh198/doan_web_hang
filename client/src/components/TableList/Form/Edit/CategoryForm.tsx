...
import { updateCategory } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
import { useTranslations } from "next-intl";
import { getDataCategoryServer } from "@/lib/callAPI/admin/ServiceReduxCallAPI"
...
export default function DefaultInputs({ dataDetail }: { dataDetail: any }) {
  ...
  const createData = async (values: any, resetForm: () => void) => {
    ...
    const data = await dispatch(updateCategory(dataDetail?.id, formData, true));
  ...
  return (
    <ComponentCard title={t('Base.EditTitle', { field: t('Page.category') })}>
      <Formik
        enableReinitialize
        initialValues={{
