...
import { updatePublisher } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
...
export default function DefaultInputs({ dataDetail }: { dataDetail: any }) {
  ...
  const PublisherSchema = Yup.object().shape({
    ...
    status: Yup.string()
      .required(t('Validation.RequiredField', { field: t('Publisher.status') })),
  });
  ...
  const createData = async (values: any, resetForm: () => void) => {
    const data = await dispatch(updatePublisher(dataDetail?.id, values, true));
  ...
  return (
    <ComponentCard title={t('Base.EditTitle', { field: t('Page.publisher') })}>
      <Formik
       enableReinitialize
      initialValues={{
