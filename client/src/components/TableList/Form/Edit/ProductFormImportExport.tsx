...
import { updateProductImportExport } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
import { useTranslations } from "next-intl";
import { getDataProductClient} from "@/lib/callAPI/admin/ServiceReduxCallAPI"
...
export default function DefaultInputs({ dataDetail }: { dataDetail: any }) {
  ...
  const [check, setCheck] = useState<boolean>(false);
   const [formData, setFormData] = useState({
          type: dataDetail.type ??  IMPORT.toString(),
          mode: dataDetail.mode ?? RENT.toString(),
          quantity: dataDetail.quantity ?? 0,
          import_price: dataDetail.import_price ?? 0,
          action: dataDetail.action ?? IMPORT.toString(),
          expected_sell_price: dataDetail.expected_sell_price ?? 0,
          expected_rent_price: dataDetail.expected_rent_price ?? "",
          actual_price_at_that_time: dataDetail.actual_price_at_that_time ?? 0,
          product_id:  dataDetail.product_id ?? 0,
          description: dataDetail.type ?? null,
        });
  ...
  const updateData = async (values: any, resetForm: () => void) => {
    if(values?.product_id === 0) {
        toast.error("Vui lòng chọn sản phẩm trước khi thêm!");
      return ;
    }
    const data = await dispatch(updateProductImportExport(dataDetail?.id,values));
    if (data?.status === "success") {
      ...
      if (result.isConfirmed) {
        router.push("/admin/manage-list?query=product-import-export");
      } else {
        resetForm();
      }
    }
  ...
  return (
    <ComponentCard title={t('Base.CreateTitle', { field: t('Page.productImportExport') })}>
      ...
      enableReinitialize   
        ...
        onSubmit={(values, { resetForm }) => updateData(values, resetForm)}
      >
