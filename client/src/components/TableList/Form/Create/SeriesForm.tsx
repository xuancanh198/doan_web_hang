...
import { createSeries } from "@/lib/callAPI/admin/ServiceReduxCallAPI";
...
export default function DefaultInputs() {
  ...
  const SeriesSchema = Yup.object().shape({
    code: Yup.string()
      .required(t('Validation.RequiredField', { field: t('Series.code') }))
      .min(2, t('Validation.MinLength', { field: t('Series.code'), min: 2 }))
      .max(255, t('Validation.MaxLength', { field: t('Series.code'), max: 20 })),

    name: Yup.string()
      .required(t('Validation.RequiredField', { field: t('Series.name') }))
      .min(2, t('Validation.MinLength', { field: t('Series.name'), min: 2 }))
      .max(255, t('Validation.MaxLength', { field: t('Series.name'), max: 50 })),

    status: Yup.string()
      .required(t('Validation.RequiredField', { field: t('Series.status') })),
       image: Yup.mixed()
          .required(t('Validation.RequiredField', { field: t('Series.image') }))
  ...
  const options = [
    { value: STATUS_ACTIVE, label: t('Series.statusValue.active') },
    { value: STATUS_NOT_ACTIVE, label: t('Series.statusValue.hideTemporarily') },
  ];
  ...
  const createData = async (values: any, resetForm: () => void) => {
    ...
    const data = await dispatch(createSeries(formData));
   ...
   const result = await Swal.fire({
     ...
     if (result.isConfirmed) {
        router.push("/admin/manage-list?query=series");
      } else {
  ...
  return (
    <ComponentCard title={t('Base.CreateTitle', { field: t('Page.series') })}>
      <Formik
        initialValues={{
          code: "",
          name: "",
          ...
          return (
            <Form>
              <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-12 md:col-span-4">
                  <Label>{t('Series.status')}</Label>
                  <div className="relative">
                    <Select
                      value={values.status}
                      options={options}
                      placeholder={t('Series.changeTextPlaceholder', { field: t('Series.status') })}
